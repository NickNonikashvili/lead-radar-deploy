...
(function (global) {
  ...
  function paintPlan(root) {
    ...
    const tbl = $('#td-classes', root); if (tbl) tbl.innerHTML = ids.map(id => { const C = Courses[id]; const d = dataOf(id); const w = weakest(id); const dc = dueCards(id); const ex = App.nextExamOf ? App.nextExamOf(C) : null; const st = App.streakOf ? App.streakOf(d) : 0; return `<tr><td><a class="chip course-${id}" href="#/${id}">${esc(C.short)}</a></td><td class="num">${st ? `${icon('fire', 12)} ${st}` : '—'}</td><td class="num">${dc.count + dc.estimated ? `<button class="linkish" data-action="td-review-one" data-c="${id}">${dc.count + dc.estimated} due</button>` : '0'}</td><td>${w ? `<a href="#/${id}/lesson?topics=${encodeURIComponent(w.t)}">${esc(w.label)}</a>` : '<span class="muted">—</span>'}</td><td>${ex ? `${esc(ex.name)} · ${esc(ex.dateLabel || App.fmtDate(ex.date))}` : '<span class="muted">none</span>'}</td></tr>`; }).join('');
  }
  App.views.today = {
    title: 'Today',
