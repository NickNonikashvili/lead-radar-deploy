...
(function (global) {
  ...
  const REASONS = [['wrong', 'The answer or solution is wrong'], ['typo', 'Typo or formatting problem'], ['unclear', 'The question or note is unclear'], ['bug', 'Something did not work'], ['other', 'Something else']];
  ...
  App.reportIssue = function (ctx) {
    ...
    el.innerHTML = `<form class="modal report-modal" role="dialog" aria-label="Report a problem"><div class="row between mb-1"><div class="panel-title">${icon('flag')} Report a problem</div><button type="button" class="icon-btn" data-action="close" aria-label="Close">${icon('x', 14)}</button></div>
      ...
      <div class="field"><label for="rp-reason">What is wrong?</label><select class="select" id="rp-reason">${REASONS.map(([v, l]) => `<option value="${v}">${l}</option>`).join('')}</select></div>
      <div class="field mt-1"><label for="rp-text">Tell us more <span class="muted">(optional)</span></label><textarea class="input" id="rp-text" rows="3" maxlength="800" placeholder="What did you expect instead?"></textarea></div>
