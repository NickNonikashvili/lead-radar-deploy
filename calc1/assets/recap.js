...
(function (global) {
  ...
  App.recapCanvas = function (s) {
    ...
    // day dots
    s.days.forEach((d, i) => { const on = s.active.includes(d); x.beginPath(); x.arc(110 + i * 132, 430, 34, 0, Math.PI * 2); x.fillStyle = on ? '#F2C14E' : 'rgba(255,255,255,0.18)'; x.fill(); x.fillStyle = on ? '#1a1a1a' : 'rgba(255,255,255,0.7)'; x.font = '700 26px Inter, system-ui, sans-serif'; x.textAlign = 'center'; x.fillText(dow[i], 110 + i * 132, 440); x.textAlign = 'left'; });
    ...
    tiles.forEach(([v, l], i) => { const col = i % 3, row = Math.floor(i / 3); const px = 80 + col * 315, py = 520 + row * 215; x.fillStyle = 'rgba(255,255,255,0.12)'; roundRect(x, px, py, 290, 185, 26); x.fill(); x.fillStyle = '#fff'; x.font = '700 72px Inter, system-ui, sans-serif'; x.fillText(v, px + 26, py + 92); x.fillStyle = 'rgba(255,255,255,0.8)'; x.font = '500 26px Inter, system-ui, sans-serif'; x.fillText(l, px + 26, py + 145); });
    x.fillStyle = 'rgba(255,255,255,0.85)'; x.font = '600 30px Inter, system-ui, sans-serif'; x.fillText('mathub.space · free study hub for Montana State classes', 80, 1010);
    return c;
  };
