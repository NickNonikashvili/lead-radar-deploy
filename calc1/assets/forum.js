  ...
  App.views.forum = {
    ...
    async inbox(box) {
      ...
      try {
        ...
        el.innerHTML = r.notifications.length ? `<div class="panel" style="padding:6px 8px">${r.notifications.map(n => `<a class="inbox-row${n.read ? '' : ' unread'}" href="${n.kind === 'badge' ? '#/badges' : threadLink(n.post_id)}" data-action="open-notif" data-id="${n.id}"><span class="inbox-dot"></span><div class="inbox-body"><div><b>${esc(n.actor)}</b> ${n.kind === 'badge' ? 'awarded you' : n.kind === 'reply' ? 'replied to your comment on' : n.kind === 'accepted' ? 'accepted your answer on' : 'commented on your post'} <b>${esc(n.title)}</b></div><div class="small muted">${esc(n.snippet)}</div></div><span class="small muted">${timeAgo(n.created)}</span></a>`).join('')}</div>` : '<div class="empty">Nothing yet. When someone answers one of your posts, it shows up here.</div>';
      } catch (e) { el.innerHTML = `<div class="empty">${esc(e.message)}</div>`; }
    },
